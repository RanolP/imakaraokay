...
const AdminInterface: Component<AdminInterfaceProps> = (props) => {
  ...
  const handleEditSong = (song: Song) => {
    // If already editing this song, collapse the form
    if (editingSong()?.id === song.id && showSongForm()) {
      resetSongForm();
      return;
    }
  ...
  const handleEditArtist = (artist: Artist) => {
    // If already editing this artist, collapse the form
    if (editingArtist()?.id === artist.id && showArtistForm()) {
      resetArtistForm();
      return;
    }
  ...
  const handleAddSong = () => {
    // If already showing add form, collapse it
    if (!editingSong() && showSongForm()) {
      resetSongForm();
      return;
    }
  ...
  const handleAddArtist = () => {
    // If already showing add form, collapse it
    if (!editingArtist() && showArtistForm()) {
      resetArtistForm();
      return;
    }
      ...
      <Show when={activeTab() === 'songs'}>
        <div class="space-y-6">
                      ...
                      <div class="p-4 hover:bg-gray-50">
                        <div class="flex justify-between items-start">
                          ...
                          <div class="flex space-x-2">
                            ...
                            >
                              {editingSong()?.id === song.id && showSongForm() ? 'Cancel' : 'Edit'}
                            </button>
                            <button
          ...
          <div class="flex justify-center">
            ...
            >
              {!editingSong() && showSongForm() ? 'Cancel' : 'Add Song'}
            </button>
          </div>

          {/* Add Song Form - appears below the button */}
          <Show when={!editingSong() && showSongForm()}>
            <SongForm />
          </Show>
        </div>
      </Show>
        ...
        <div class="space-y-6">
          ...
          <div class="flex justify-between items-center">
            ...
            <button
              ...
              class="bg-purple-600 text-white px-4 py-2 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
              {!editingArtist() && showArtistForm() ? 'Cancel' : 'Add Artist'}
            </button>
          </div>

          {/* Add Artist Form - appears below the button */}
          <Show when={!editingArtist() && showArtistForm()}>
            <ArtistForm />
          </Show>
